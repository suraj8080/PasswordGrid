...
public class StreakView extends View {
    ...
    private class OnTouchProcessedListener implements TouchProcessor.OnTouchProcessed {
        ...
        public void onMove(MotionEvent event) {
            ...
            /*if(line.getStartIndex().row != rowIdx && line.getStartIndex().col != colIdx) {
                *//*
                check direction here and apply changes in row,col accordingly to end previous line end position
                 *//*

                // it works when row increases and col decreasing (right to left) we can replace it by checking direction of lines
                if(rowIdx> line.getStartIndex().row && colIdx< line.getStartIndex().col) {
                    line.end.set(mGrid.getCenterColFromIndex(colIdx), mGrid.getCenterRowFromIndex(rowIdx - 1));

                    StreakView.StreakLine newStreakLine = new StreakView.StreakLine();
                    newStreakLine.start.set(mGrid.getCenterColFromIndex(colIdx), mGrid.getCenterRowFromIndex(rowIdx-1));
                    newStreakLine.getStartIndex().set(rowIdx, colIdx);
                    //newStreakLine.getEndIndex().set(rowIdx+1,colIdx);
                    newStreakLine.setColor(line.getColor());
                    //newStreakLine.startIdx.set(rowIdx+1, colIdx);
                    //newStreakLine.start.set(event.getX() , event.getY());
                    Log.d("oldStreakLine colIdx " + colIdx, " rowIdx " + rowIdx);
                    Log.d("newStreakLine start colIdx " + newStreakLine.getStartIndex().col, " rowIdx " + newStreakLine.getStartIndex().row);
                    Log.d("newStreakLine end colIdx " + newStreakLine.getEndIndex().col, " rowIdx " + newStreakLine.getEndIndex().row);
                    mLines.push(newStreakLine);
                    line = mLines.peek();
                }else if(rowIdx> line.getStartIndex().row && colIdx> line.getStartIndex().col){
                    // it works when row increases and col increasing(left to right) we can replace it by checking direction of lines
                    line.end.set(mGrid.getCenterColFromIndex(colIdx-1), mGrid.getCenterRowFromIndex(rowIdx ));

                    StreakView.StreakLine newStreakLine = new StreakView.StreakLine();
                    newStreakLine.start.set(mGrid.getCenterColFromIndex(colIdx-1), mGrid.getCenterRowFromIndex(rowIdx));
                    newStreakLine.getStartIndex().set(rowIdx, colIdx);
                    //newStreakLine.getEndIndex().set(rowIdx+1,colIdx);
                    newStreakLine.setColor(line.getColor());
                    //newStreakLine.startIdx.set(rowIdx+1, colIdx);
